...
// Monitor
class LE {
  private int leit, escr, escrEspera;  
  
  // Construtor
  LE() { 
     this.leit = 0;
     this.escr = 0;
     this.escrEspera = 0; //define o numero de threads esperando para escrita
  } 
  
  // Entrada para leitores
  public synchronized void EntraLeitor (int id) {
    try { 
      while (this.escr > 0 || this.escrEspera > 0) { //bloqueia caso haja alguma thread na espera da escrita
      //if (this.escr > 0) {
  ...
  public synchronized void EntraEscritor (int id) {
    try { 
      this.escrEspera++; //Sinaliza que há um thread de escrita
      while ((this.leit > 0) || (this.escr > 0)) {
         System.out.println ("le.escritorBloqueado("+id+")");
         wait();  //bloqueia pela condicao logica da aplicacao 
      }
      this.escrEspera--; //sinaliza que não há mais espera na escrita
      this.escr++;
